...
import type { EffectiveCollectionOptions } from './options.js'
...
function slash(npath: string, sep: string) {
  return npath.replace(windowsSlashRE, sep);
}

function normalizePath(npath: string) {
...
export const replaceExt = (npath: string, ext: string) => {
...
export const isValidRelativeLink = (link: string) => {
  if (!link) {
    return false;
  }
...
export const splitPathFromQueryAndFragment = (url: string): [string, string | null] => {
...
export const normaliseAstroOutputPath = (
  initialPath: string,
  collectionOptions: Pick<EffectiveCollectionOptions, "base">,
) => {
  const buildPath = () => {
...
export const generateSlug = (pathSegments: string[]) => {
  return pathSegments
...
export const shouldProcessFile = (npath: string) => {
...
export const getMatter = (npath: string): MatterData => {
  const readMatter = () => {
...
export const resolveCollectionBase = (
  collectionOptions: Pick<EffectiveCollectionOptions, "collectionBase" | "collectionName">,
) => {
